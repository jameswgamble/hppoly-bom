...
// script.js – HP | Poly Configurator – v10.19: fixed TAA/JITC path
// Features: E70 power option (wall/injector), short legalese, promo button, correct MSRP totals
...
async function init() {
  ...
  function generate() {
    ...
    if (taaJitc) {
      ...
      const tc10Sku = () => pick("973F9AA", "977L6AA");

      if (isUSBorPC) {
        if (roomSize === "Small") {
          addLine(results, "B95SPAA");
          addSupport(results, "v12", supportTerm);
        } else if (roomSize === "Medium") {
          addLine(results, pick("A09D6AA", "A09D5AA"));
          addSupport(results, "v52", supportTerm);
        } else {
          addLine(results, pick("AV1E4AA", null));
        ...
        if (typeOfSystem === "Windows PC based solution") {
          if (platform === "Microsoft Teams") {
            if (roomSize === "Small" || roomSize === "Medium") {
              addLine(results, "DS1R6AW");
            } else {
              addLine(results, "DS0W9AW");
            }
        ...
        if (roomSize === "Small") {
          addLine(results, pick("A3SW0AA", "A3SV9AA"));
          ...
          addLine(results, pick("A4MA2AA", "A4MA1AA"));
          addSupport(results, "x72", supportTerm);
          addLine(results, tc10Sku());
          addSupport(results, "tc10", supportTerm);
        } else {
          addLine(results, pick("99T11AA", "99T10AA"));
...

      {
        ...
        if (isG62 || isX52 || isX72) {
          ...
          if (cam === "E60") {
            addLine(results, "9W1A7AA");
            addSupport(results, "e60", supportTerm);
          } else if (cam === "E70") {
            addLine(results, pick("886C9AA", "886C8AA"));
            addSupport(results, "e70", supportTerm);
            const e70Pwr = document.getElementById("e70Power")?.value || "None";
            if (e70Pwr === "Wall" && !hasSku(results, "875K6AA")) {
              addLine(results, "875K6AA", "Poly E70 wall / external power supply (12V 5A)");
            }
            if (e70Pwr === "Injector" && !hasSku(results, "85X03AA#ABA")) {
              addLine(results, "85X03AA#ABA", "Poly E70 / Trio PoE midspan injector kit");
            }
          }
        }
      }
    ...
    if (!taaJitc) {
      if (isUSBorPC) {
        if (roomSize === "Small") {
          addLine(results, "A9DD8AA#ABA");
          addSupport(results, "v12", supportTerm);
        } else if (roomSize === "Medium") {
          addLine(results, "A09D4AA#ABA");
          addSupport(results, "v52", supportTerm);
        } else {
        ...
        if (roomSize === "Small") {
          addLine(results, "A3SV5AA#ABA");
          addSupport(results, "x32", supportTerm);
          if (!hasSku(results, "875K5AA")) addLine(results, "875K5AA");
          addSupport(results, "tc10", supportTerm);
        } else if (roomSize === "Medium") {
          addLine(results, "8D8K2AA#ABA");
          addSupport(results, "x52", supportTerm);
          addLine(results, "875K5AA");
          addSupport(results, "tc10", supportTerm);
        } else if (roomSize === "Large") {
          addLine(results, "A4LZ8AA#ABA");
          addSupport(results, "x72", supportTerm);
          addLine(results, "875K5AA");
          addSupport(results, "tc10", supportTerm);
        } else {
