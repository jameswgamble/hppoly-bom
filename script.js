...
// script.js – HP | Poly Configurator – v10.17 restore
...
async function init() {
  ...
  if (!res.ok) throw new Error('Failed to load skus_merged.json');
  ...
  promoWrap.innerHTML = '<div class="font-semibold text-zinc-900">📢 Featured configuration</div><p class="text-sm text-zinc-800">Microsoft Teams · Android appliance · Medium room · 3yr Poly+ · Poly E70 AI camera (auto-tracking / camera switching)</p><p class="text-xs text-zinc-700">Kit only · works with Microsoft Teams, Zoom, or Google Meet · <a href="https://youtu.be/2AX-8x6CWN0?si=8O1Vp7uUVrohw1j1" target="_blank" rel="noopener" class="underline font-medium">X52 + E70 reference video</a></p><button type="button" id="applyPromoBtn" class="px-3 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-white text-sm font-medium rounded shadow-sm">Apply this config & generate BOM</button>';
  ...
  e70PowerWrap.innerHTML = '<label class="block font-medium">E70 power option</label><select id="e70Power" class="border p-2 w-full"><option value="None">None — using existing PoE+ switch</option><option value="Wall">Wall power supply (875K6AA)</option><option value="Injector">PoE midspan injector kit (85X03AA#ABA)</option></select>';
  ...
  function updateE70PowerVisibility(){
    ...
    if (wrap) wrap.classList.toggle("hidden", cam !== "E70");
  }
  ...
  function generate(){
    ...
    html += '<th class="border px-4 py-2 text-left">Qty</th><th class="border px-4 py-2 text-left">SKU</th><th class="border px-4 py-2 text-left">Description</th>';
    ...
    results.forEach(r => {
      ...
      html += '<tr><td class="border px-4 py-2">' + r.quantity + '</td><td class="border px-4 py-2">' + sku + '</td><td class="border px-4 py-2">' + r.description + '</td>';
      if (includePrices) html += '<td class="border px-4 py-2">' + fmtCurrency(r.msrp) + '</td>';
      html += '</tr>';
    });
